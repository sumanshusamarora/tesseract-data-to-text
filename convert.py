...
def generate_text_from_ocr_output(ocr_output_path:str, text_join_delimiter="\n", overlap=.3):
    """
    ...
    for i in data_indexes:
        if i not in ignore_index and ocr_dataframe["text"][i] and ocr_dataframe["text"][i].strip() != "":
            this_row_bottom = ocr_dataframe["top"][i] + ocr_dataframe["height"][i]
            line_index = list(ocr_dataframe[(~ocr_dataframe.index.isin(ignore_index)) & (ocr_dataframe["top"] <= this_row_bottom - (overlap * ocr_dataframe["height"][i])) & (ocr_dataframe["bottom"] >= ocr_dataframe["top"][i] + (overlap * ocr_dataframe["height"][i]))].sort_values(by="left").index)
    ...
    text_list = [" ".join([ocr_dataframe["text"][index] for index in line_index if ocr_dataframe["text"][index]]) for line_index in line_indexes]
...
def stitch_ocr_files(filepaths:list, text_join_delimiter="\n"):
    ...
    for filepath in filepaths:
        ocr_text = generate_text_from_ocr_output(
            ocr_output_path=filepath,
            text_join_delimiter=text_join_delimiter,
        )
